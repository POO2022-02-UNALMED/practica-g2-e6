...
public class Conexion {
	...
	public boolean guardar(Usuario usuario) {
		...
		if (BuscarUsuario(usuario.getEmail()) == false) {
			File file = new File("src\\baseDatos\\temp\\Usuarios.txt");
			if (!file.exists()) {
				try {
					file.createNewFile();
					FileWriter writer = new FileWriter(file);
					writer.write(usuario.getNombre() + "|" + usuario.getEmail() + "|" + usuario.getFechaNacimiento()
							+ "|" + usuario.getFechaIngreso() + "|" + usuario.getClave() + "\n");
					writer.close();
	...
	public boolean leerUsuario() {
		try {
			BufferedReader bf = new BufferedReader(new FileReader("src\\baseDatos\\temp\\Usuarios.txt"));
			String bfRead;
